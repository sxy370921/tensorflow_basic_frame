...
# 结构说明：
# 这是基于LetNet改进的CNN结构,两个卷积层两个池化层，之后跟两个全连接层，全连接层都dropout。
# 在conv1取32个5*5的核，con2取64个5*5的核，pool1和pool2都是2*2的情况，全连接层都是256的情况下
# 并且参数learning_rate = 0.0007，num_steps = 6000，dropout = 0.5 的情况下最终99.3%
def cnn2(tf_x, dp, is_training):
    ...
    h1 = tf.layers.dense(fc, 256, activation=tf.nn.relu)
    d1 = tf.layers.dropout(h1, rate=dp, training=is_training)
    h2 = tf.layers.dense(d1, 256, activation=tf.nn.relu)
    d2 = tf.layers.dropout(h2, rate=dp, training=is_training)
    output = tf.layers.dense(d2, 10)  # output layer
...
outputs = cnn2(X, dropout, tf_is_training)
